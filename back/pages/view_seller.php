      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Seller</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">View Seller</h6>
        </nav>
          ...
          <div class="card">
            ...
            <div class="card-body pt-4 p-3">
              <ul class="list-group">
                        <?php
                            $sid=$_GET['sid'];
                            $sel="SELECT * FROM seller WHERE s_id='$sid'";
                            $result=mysqli_query($con,$sel);
                            while($row=$result->fetch_assoc()){
                        ?>
                    <h6 class="mb-0"><?php echo $row['s_name']; ?> Details</h6>
                    <li class="list-group-item border-0 d-flex p-4 mb-2 bg-gray-500 border-radius-lg">
                      <div class="d-flex flex-column">
                        ...
                        <span class="mb-2 text-xs">Email Address: <span class="text-dark ms-sm-2 font-weight-bold"><?php echo $row['s_email']; ?></span></span>
                        <span class="text-xs">Phn Number: <span class="text-dark ms-sm-2 font-weight-bold"><?php echo $row['s_phone']; ?></span></span>
                      </div>
                      <div class="ms-auto text-end">
                        <a class="btn btn-link text-danger text-gradient px-3 mb-0" href="delete_seller.php?sid=<?php echo $row['s_id']; ?>"><i class="material-icons text-sm me-2">delete</i>Delete</a>
                ...
                <h6 class="mb-0">Offer Details</h6>
                    <?php
                        $sel="SELECT veg_offer.*, seller.s_name, seller.s_id
                        FROM veg_offer
                        JOIN seller ON seller.s_name = veg_offer.seller_name AND s_id='$sid'";
                        $result=mysqli_query($con,$sel);
                        while($row=$result->fetch_assoc()){
                    ?>
